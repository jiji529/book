...
<body>
	<div class="container">
			...
			<table class="table table-sm table-striped">
				...
				<tbody>
					<c:if test="${cart == null || cart.size() < 1 }">
						<tr>
							<td colspan="4">장바구니에 도서가 없습니다.</td>
						</tr>
					</c:if>
					...
					<c:forEach var="item" items="${cart}">
						...
						<tr data-bookid="${item.key}">
							...
							<td>${item.key}</td>
							<!-- ${books.get(item.key)}가 책 한 권의 정보 -->
							<td>${books.get(item.key).bookname}</td>
							<!-- date-value : 변경되기 전 값 저장 -->
							<td><input type="number" value="${item.value}" name="amount" data-value="${item.value}"><i class="bi bi-check hide"></i></td>
							<td class="price" data-price="${books.get(item.key).price}">${books.get(item.key).price}원</td>
							<td class="itemprice" data-itemprice="${itemprice}">${itemprice}원</td>
		...
		<div>
			...
			<button class="btn btn-sm btn-warning" id="update_all">일괄 변경</button>
